...
for test_case in range(3):
    ...
    d1 = pd.read_csv('trace_wo_red.csv',delimiter=',',index_col=['T_ref'], skiprows = lambda x: x not in range(start_indx, end_indx) and x !=0, usecols=['injected','virtual','serial','disable'])
    d2 = pd.read_csv('trace_w_red.csv',delimiter=',',index_col=['T_ref'], skiprows = lambda x: x not in range(start_indx, end_indx) and x !=0,usecols=['injected','virtual','serial','disable'])
    d3 = pd.read_csv('weight-based_w_red_int.csv',delimiter=',',index_col=['T_ref'], skiprows = lambda x: x not in range(start_indx, end_indx) and x !=0,usecols=['injected','virtual','serial','disable'])
    d4 = pd.read_csv('weight-based_w_red_ext.csv',delimiter=',',index_col=['T_ref'], skiprows = lambda x: x not in range(start_indx, end_indx) and x !=0,usecols=['injected','virtual','serial','disable'])
    d5 = pd.read_csv('weight-based_w_red_hyb.csv',delimiter=',',index_col=['T_ref'], skiprows = lambda x: x not in range(start_indx, end_indx) and x !=0,usecols=['injected','virtual','serial','disable'])
    d6 = pd.read_csv('ford-fulkerson_w_red_int.csv',delimiter=',',index_col=['T_ref'], skiprows = lambda x: x not in range(start_indx, end_indx) and x !=0,usecols=['injected','virtual','serial','disable'])
    d7 = pd.read_csv('ford-fulkerson_w_red_ext.csv',delimiter=',',index_col=['T_ref'], skiprows = lambda x: x not in range(start_indx, end_indx) and x !=0,usecols=['injected','virtual','serial','disable'])
    d8 = pd.read_csv('ford-fulkerson_w_red_hyb.csv',delimiter=',',index_col=['T_ref'], skiprows = lambda x: x not in range(start_indx, end_indx) and x !=0,usecols=['injected','virtual','serial','disable'])
